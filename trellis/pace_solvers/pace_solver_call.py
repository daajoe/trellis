...
# TODO:
def compute_decomp(instance, solver_path=os.path.join(os.path.expanduser("~"),
                                                      'src/treewidth-portfolio/solvers/exact/tcs-meiji/tw-exact'),
                   solver_args = '', timeout=30):
    solver_name = solver_path
    call = 'timeout %i %s %s < %s' % (timeout, solver_name, solver_args.replace('"', '').replace("&quot;",""), instance)
    sys.stderr.write('%s\n' % call)
